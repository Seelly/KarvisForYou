...
SKILL_PROMPT_LINES = {
    ...
    "feishu.docs.create": '**feishu.docs.create** `{title, content?, folder_token?}` — 创建新的飞书云文档（用户说"帮我创建一个飞书文档"、"新建一个文档叫XX"时触发。title=文档标题；content=可选的初始内容，支持 Markdown 格式；folder_token=可选的目标文件夹 token）',
    "feishu.wiki.create": '**feishu.wiki.create** `{title, space_id?, content?, parent_node_token?}` — 在飞书知识库中创建文档（用户说"在知识库里新建一个文档"、"帮我在wiki里写一篇XX"时触发。title=文档标题；space_id=知识库ID，不传则列出可用知识库；content=可选初始内容，支持 Markdown；parent_node_token=可选父节点token，不传则在根目录创建）',
    "feishu.docs.read": '**feishu.docs.read** `{source, url?, document_id?, wiki_node_token?, file_path?, max_chars?, max_blocks?}` — 读取飞书云文档/知识库/云盘内容（source: docx/wiki/drive；drive 需 file_path）',
    "feishu.docs.write": '**feishu.docs.write** `{source, url?, document_id?, wiki_node_token?, block_id?, file_path?, content, mode?}` — 写入飞书云文档/知识库/云盘（docx/wiki: append 或 patch(需block_id)；drive: append/replace）',
    "feishu.task.confirm": '**feishu.task.confirm** `{index? , task_guid?}` — 当匹配到多个飞书任务时，确认并完成指定任务',
